...
def stress_assessment(request):
    ...
    sender_id = request.session.get('email', 'anonymous')
    ...
    if request.method == "POST":
        ...
        request.session['conversation_history'].append({'sender': 'user', 'message': user_message})
    else:
        user_message = "Start the assessment"
        request.session['conversation_history'] = []
    ...
    # Variables to send to template on completion
    stress_score = ''
    risk_level = ''
    ...
    try:
        ...
        for msg in rasa_data:
            if 'text' in msg:
                text = msg['text']
                request.session['conversation_history'].append({'sender': 'bot', 'message': text})

                if text.startswith("Assessment Results:"):
                    assessment_result = text

                if any(qword in text.lower() for qword in ['happiness', 'enjoy the little things', 'quote:']):
                    motivational_quote = text

                if "spotify.com" in text:
                    parts = re.findall(r'(https://open\.spotify\.com/[^\s]+)', text)
                    if parts:
                        song_link = parts[-1]

                if "youtube.com" in text or "youtu.be" in text:
                    parts = re.findall(r'(https://www\.youtube\.com/[^\s]+|https://youtu\.be/[^\s]+)', text)
                    if parts:
                        video_link = parts[-1]

                if any(key in text for key in ['Psychologists', 'Emergency Contacts', 'Psychiatrists']):
                ...
                if 'Options:' in text:
                    ...
                    current_options = [{'number': num, 'text': opt.strip()} for num, opt in option_matches]

                elif (not assessment_result and not motivational_quote and not song_link and not video_link):
                    current_question = text

        # Parse score and risk level from assessment_result
        if assessment_result:
            score_match = re.search(r'(\d+/\d+ \(\d+\.?\d*%\))', assessment_result)
            risk_match = re.search(r'Risk Level:\s*(.*)', assessment_result)

            stress_score = score_match.group(1) if score_match else ''
        ...
        import traceback
        print("Rasa connection error in Django:", e)
        traceback.print_exc()
        current_question = "Connection error with Rasa server."
        current_options = []
        assessment_result = None
    ...
    context = {
        ...
        'is_assessment_complete': assessment_result is not None,
    }

    return render(request, 'stress_assessment.html', context)
